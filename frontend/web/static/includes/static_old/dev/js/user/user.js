var UserController = (function ($) {
    return {
        createUpdate: function () {
            UserController.CreateUpdate.init();
        }
    };
}(jQuery));

UserController.CreateUpdate = (function ($) {
    var isSent = 0;
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success === 1) {
                        ...
                        $modal.modal('show');
                        StateController.initializeChozen();
                        $('#editUserForm, #reset-password-form').unbind().on('submit', function (e) {
                            e.preventDefault();
                            if(isSent == 1) {
                                return;
                            }
    ...
    var createUpdateClass = function () {
        ...
        $('#newUserForm').on('beforeSubmit', function (e) {
            ...
            success: function (data) {
                if (data.success == '1') { 
                    ...
                    $.each(data.errors, function(key, val) {
                        $( "#"+formId+" .field-userform-"+key ).addClass('has-error').find('p').text(val);
                    });
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
