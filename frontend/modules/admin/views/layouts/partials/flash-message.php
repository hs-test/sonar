...
<?php foreach (Yii::$app->session->getAllFlashes() as $key => $message): ?>
    <?php if(is_array($message)): ?>
        <?php foreach($message as $msg): ?>
            <div class="alert <?=($msg['type'] === 'success') ? 'alert-success' : 'alert-danger' ?> alert-dismissible fade in" role="alert">
                <button class="close" aria-label="Close" data-dismiss="alert" type="button"><span aria-hidden="true">×</span></button>
                <p><?= $msg['text'] ?></p>
            </div>
        <?php endforeach;?>
    <?php else: ?>
    <div class="alert <?=($key === 'success') ? 'alert-success' : 'alert-danger' ?> alert-dismissible fade in" role="alert">
        <button class="close" aria-label="Close" data-dismiss="alert" type="button"><span aria-hidden="true">×</span></button>
        <p><?= $message ?></p>
    </div>
    <?php endif;?>
